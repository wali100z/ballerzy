...
export default function GalleryPreview() {
  ...
  const handleClick = () => {
    navigate("/gallery"); // Navigates to full gallery page
  };
    ...
    <section className={styles.gallery}>
      ...
      <div className={styles.grid}>
        {images.slice(0, 6).map((img, i) => (
          <img key={i} src={img} alt={`Gallery preview ${i + 1}`} onClick={handleClick} />
        ))}
      </div>

      <button className={styles.viewAllBtn} onClick={handleClick}>
        Se hele galleriet
      </button>
    </section>
  );
